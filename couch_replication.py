...
def replicate_db(
    ...
    dst = 'http://{}:{}@{}/{}'
    dst = dst.format(credentials_target[0], credentials_target[1],
                     dst_url, db_name)
    request_dict = {'source': src_url + db_name, 'target': dst}

    # print(json.dumps(request_dict))

    replicate_request = requests.post(src_url + '_replicate',
            data=json.dumps(request_dict), headers=headers,
            auth=HTTPBasicAuth(credentials_source[0],
            credentials_source[1]), timeout=(30, 120))
...
def create_db(dburl, dbname, credentials):
    ...
    if dburl.endswith('/'):
        ...
        dburl = dburl + '/' + dbname_enc
    check_get = requests.get(dburl, auth=HTTPBasicAuth(credentials[0],
                             credentials[1]))
    ...
    if dbname in check_get.content:
        ...
        r = requests.put(dburl, auth=HTTPBasicAuth(credentials[0],
                         credentials[1]))
        if r.reason == 'Created':
            return True
        else:
...
def delete_db(
    ...
    d = requests.delete(db_url + db_name, auth=HTTPBasicAuth(user,
                        password))
    return d
...
parser.add_argument('-s', '--source', action='store',
                    help='IP of source couchdb, in format IP:PORT')
parser.add_argument('-t', '--target', action='store',
                    help='IP of target couchdb, in format IP:PORT')
parser.add_argument('-as', '--authsource', action='store',
                    help='path to file with credentials for source Couchdb'
                    )
...
parser.add_argument('-d', '--dbname', action='store',
                    help='database name for replication only single database'
                    )
parser.add_argument('-at', '--authtarget', action='store',
                    help='path to file with credentials for target Couchdb'
                    )
...
if args.dbname is None:
    ...
    src_dbs = get_all_db(db_source, credentials_source[0],
    ...
    try:
        for i in range(len(src_dbs)):
            ...
            rep_req = replicate_db(src_dbs[i], db_source, db_target_ip,
                                   credentials_source,
                                   credentials_target)
            if 'error' in rep_req:
                log_err = 'Replication error: {}, reason: {}'
                log_err = log_err.format(rep_req['error'],
                        rep_req['reason'])
                logger.error(log_err)
            elif 'ok' in rep_req:
                if 'session_id' in rep_req and 'history' in rep_req:
                    log_srt = \
                        'Replication success: session_id: {}, start_time: {} ,end_time: {}, missing_checked: {}, missing_found: {}, docs_read: {}, docs_written: {}, doc_write_failures: {}'
                    log_srt = log_srt.format(
                        rep_req['session_id'],
                        rep_req['history'][0]['start_time'],
                        rep_req['history'][0]['end_time'],
                        rep_req['history'][0]['missing_checked'],
                        rep_req['history'][0]['missing_found'],
                        rep_req['history'][0]['docs_read'],
                        rep_req['history'][0]['docs_written'],
                        rep_req['history'][0]['doc_write_failures'],
                        )
                    logger.info(log_srt)
                else:
...
# Delete DB that is not in prod already
    ...
    try:
        ...
        trg_dbs = get_all_db(db_target, credentials_target[0],
                             credentials_target[1])
        ...
        if len(res_list) > 0:
            ...
            for i in range(len(res_list)):
                del_res = delete_db(db_target, res_list[i],
                                    credentials_target[0],
                                    credentials_target[1])

               # print type(del_res.status_code)
               # print type(res_list[i])

                logger.info('Delete: %s , result: %d'
                            % (str(res_list[i]), del_res.status_code))
                if del_res.status_code != 200:
                    logger.error('Error deleting DB: %s, reslut %d'
                                 % (str(res_list[i]),
                                 del_res.status_code))
        else:
...
try:
    if args.dbname is not None:
        ...
        if r != True:
            ...
            replicate_request = replicate_db(db_name, db_source,
                    db_target_ip, credentials_source[0],
                    credentials_source[1])
            if 'error' in replicate_request:
                log_err = 'Replication error: {}, reason: {}'
                log_err = log_err.format(replicate_request['error'],
                        replicate_request['reason'])
                logger.error(log_err)
            elif 'ok' in replicate_request:
                if 'session_id' in replicate_request and 'history' \
                    in replicate_request:
                    log_srt = \
                        'Replication success: session_id: {}, start_time: {} ,end_time: {}, missing_checked: {}, missing_found: {}, docs_read: {}, docs_written: {}, doc_write_failures: {}'
                    log_srt = log_srt.format(
                        replicate_request['session_id'],
                        replicate_request['history'][0]['start_time'],
                        replicate_request['history'][0]['end_time'],
                        replicate_request['history'
                                ][0]['missing_checked'],
                        replicate_request['history'][0]['missing_found'
                                ],
                        replicate_request['history'][0]['docs_read'],
                        replicate_request['history'][0]['docs_written'
                                ],
                        replicate_request['history'
                                ][0]['doc_write_failures'],
                        )
                    logger.info(log_srt)
                else:
                ...
                logger.error('Unexpected result: %s'
                             % str(replicate_request))
except:
    logger.exception('Exception due replicating DB ')
    sys.exit(1)
sys.exit(0)
